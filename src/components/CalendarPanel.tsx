...
export default function CalendarPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 border-l border-slate-800 select-none">
      ...
      <div className="px-3 pt-3 pb-2.5 border-b border-slate-800 shrink-0 space-y-2">
        {/* Title row */}
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('calendar.title')}</span>
          <button onClick={() => setShowSync(v => !v)} className="text-slate-500 hover:text-slate-300 transition-colors" title="Sincronizar">
        ...
        {showSync && (
          <div className="bg-slate-900 border border-slate-700 rounded-xl p-3 space-y-2 text-xs">
            <p className="text-slate-400 font-medium">{t('calendar.sync')}</p>
            ...
            <p className="text-slate-600 text-[10px] leading-tight">{t('calendar.outlookNote')}</p>
          </div>
        )}

        {/* Task input — only the add-task field */}
        <div className="flex gap-1">
          <input
            ...
            className="flex-1 text-xs bg-slate-900 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white placeholder-slate-600 focus:outline-none focus:border-slate-500 min-w-0"
          />
          <button onClick={addTask}
            className="shrink-0 w-7 h-7 flex items-center justify-center bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-slate-400 hover:text-white transition-colors">
        ...
        <div className="flex gap-1 bg-slate-900 rounded-lg p-0.5">
          {(['month','week','day'] as const).map(v => (
            <button key={v} onClick={() => setView(v)}
              className={`flex-1 py-1 text-xs rounded-md font-medium transition-colors ${view === v ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}>
        ...
        {/* Nav */}
        <div className="flex items-center justify-between">
          <button onClick={prevPeriod} className="w-6 h-6 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 rounded transition-colors">‹</button>
          <button onClick={() => { setSelectedDate(new Date()); setCurrent(new Date(today.getFullYear(), today.getMonth(), 1)) }}
            className="text-xs font-medium text-slate-300 hover:text-white transition-colors truncate px-1 text-center">
            {headerLabel}
          </button>
          <button onClick={nextPeriod} className="w-6 h-6 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 rounded transition-colors">›</button>
        </div>
      </div>
        ...
        {view === 'month' && (
          <div className="p-2">
            <div className="grid grid-cols-7 mb-1">
              {DAY_LETTERS.map((l,i) => (
                <div key={i} className={`text-center text-[10px] font-medium py-1 ${i===0||i===6?'text-slate-600':'text-slate-500'}`}>{l}</div>
              ))}
            </div>
                ...
                return (
                  <div key={d} onClick={() => { setSelectedDate(date); setView('day') }}
                    className={`relative flex flex-col items-center py-0.5 rounded cursor-pointer transition-colors
                      ${isSel ? 'bg-blue-600' : isToday ? 'bg-slate-800' : 'hover:bg-slate-900'}
                      ${isWeekend && !isSel ? 'text-slate-600' : ''}`}>
                    <span className={`text-[11px] font-medium leading-none ${isToday && !isSel ? 'text-blue-400' : isSel ? 'text-white' : ''}`}>{d}</span>
                    {evTypes.length > 0 && (
            ...
            <div className="mt-3 pt-2 border-t border-slate-800/60 space-y-1">
              {(Object.entries(EVENT_COLORS) as [EventType, typeof EVENT_COLORS[EventType]][]).map(([type, c]) => (
                <div key={type} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-slate-500">{t(`eventTypes.${type}`)}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            {weekDays.map(day => {
              ...
              return (
                <div key={ds} onClick={() => { setSelectedDate(day); setView('day') }}
                  className={`px-2 py-1.5 rounded-lg cursor-pointer transition-colors ${isToday ? 'bg-slate-800' : 'hover:bg-slate-900'} ${isWeekend ? 'opacity-50' : ''}`}>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-semibold w-6 ${isToday ? 'text-blue-400' : 'text-slate-400'}`}>{day.getDate()}</span>
                    <span className="text-[10px] text-slate-500 uppercase w-6">{DAY_LETTERS[day.getDay()]}</span>
                    <div className="flex gap-1 flex-wrap flex-1">
                      ...
                      {evs.length === 0 && isWeekend && <span className="text-[10px] text-slate-700">{t('calendar.weekend')}</span>}
                    </div>
                  </div>
                </div>
              )
          ...
          <div className="p-3 space-y-3">
            <div className="flex items-center gap-2">
              {isSameDay(selectedDate, today) && (
                <span className="text-[10px] font-bold text-blue-400 bg-blue-900/40 px-1.5 py-0.5 rounded">{t('calendar.today')}</span>
              )}
              {!isSchoolDay(selectedDate) && (
                <span className="text-[10px] text-amber-400 bg-amber-900/30 px-1.5 py-0.5 rounded">{t('calendar.noSchool')}</span>
              )}
            </div>
            {selectedEvents.length === 0 ? (
              <p className="text-xs text-slate-600 italic">{t('calendar.noEventsDay')}</p>
            ) : (
            ...
            <div className="pt-2 border-t border-slate-800">
              <p className="text-[10px] text-slate-600 uppercase tracking-wide mb-2">{t('calendar.upcoming')}</p>
              {DEPR_EVENTS.filter(ev => new Date(ev.date + 'T00:00:00') > selectedDate).slice(0,4).map(ev => {
                const c = EVENT_COLORS[ev.type]
                return (
                  <div key={ev.id} className="flex items-start gap-2 py-1.5 border-b border-slate-800/50 last:border-0">
                    <div className={`w-1.5 h-1.5 rounded-full ${c.dot} mt-1 shrink-0`} />
                    <div>
                      <p className="text-xs text-slate-300 font-medium leading-tight">{ev.title}</p>
                      <p className="text-xs text-slate-500">{ev.date}{ev.endDate ? ` – ${ev.endDate}` : ''}</p>
                    </div>
                  </div>
                )
              })}
      ...
      {tasks.length > 0 && (
        <div className="border-t border-slate-800 shrink-0">
          <div className="px-3 pt-2 pb-1 flex items-center gap-1">
            ...
            <span className="text-[10px] text-slate-600">({tasks.filter(t=>!t.done).length}/{tasks.length})</span>
          </div>
          <div className="px-2 max-h-36 overflow-y-auto space-y-0.5 pb-2">
            {tasks.map(task => (
              <div key={task.id} className="flex items-start gap-1.5 group px-1 py-1 rounded hover:bg-slate-900/60">
                <button onClick={() => toggleTask(task.id)}
                  className={"mt-0.5 w-3.5 h-3.5 rounded border shrink-0 flex items-center justify-center transition-colors " + (task.done ? "bg-blue-600 border-blue-500" : "border-slate-600 hover:border-blue-500")}>
                ...
                <span className={"text-xs flex-1 leading-snug " + (task.done ? "line-through text-slate-600" : "text-slate-300")}>{task.text}</span>
                <button onClick={() => deleteTask(task.id)} className="opacity-0 group-hover:opacity-100 text-slate-600 hover:text-red-400 transition-all text-xs shrink-0 leading-none">&times;</button>
              </div>
            ))}
          </div>
        </div>
